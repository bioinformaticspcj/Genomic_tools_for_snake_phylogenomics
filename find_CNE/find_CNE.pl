...
my $cup_total=`grep 'physical id' /proc/cpuinfo |wc -l`;
chomp($cup_total);
...
#=head disabled by pcj 2019.4.22
	...
	#`rm tmp.map.bed` if(-e "tmp.map.bed");
	#open TMP,">>tmp.map.bed" or die "can not open tmp.map.bed\n";
	my %check_maf=();
	while(<MAF>){
		next,if/^#/;
		chomp;
	...
	#print "running mutiple_threads_GERP==============================================================================\n";
	#mutiple_threads_GERP::block_file_deal($file,$tree,$id,$seqname,$ref_species,\%block,$max_process,$cup_total) ; # added by pcj to use mutiple threads when run GERP.
	chdir "../";
...
sub filtered_bed{
	...
	while(<BEDo>){
                chomp;
                my @lines=split/\t+/;
		if($lines[-1]>$hash1{$lines[3]}){
                	$hash1{$lines[3]}=$lines[-1];
                	$hash{$lines[3]}=$_;
        	}
	}
	close BEDo;
	foreach my $CNE(keys %hash){
		my @line=split/\t/,$hash{$CNE};
	...
	my %hash4=();
	my @bed_filtered_nr = grep{++$hash4{$_}<2} @bed_filtered;
...
sub out_put_seq{
	...
	foreach my $bed_line(@{$bed}){
		...
		my $fa_name="$names[0].$names[1].$maf_num";
		$line[3]=$fa_name;
#=head		
		foreach my $seq(keys %{$maf_seq}){
			my @heads=split/\:/,$seq;
			open DEBUG,">>debug" or die "can not open debug\n";
			print DEBUG"$heads[0]\t$line[1]\t$line[2]\t$heads[1]===$heads[2]\n" if($heads[0]=~/$line[0]/);
	...
	#			print "open Bed.fa/$file/$line[3].fa\n";
		...
		close FA;
#=cut
		open IDLIST,">>IDlist.txt" or die "can not open IDlist.txt\n";
		open FINAL_BED,">>final.all.bed" or die "can not open final.all.bed\n";
		$bed_line=join"\t",@line;
		print IDLIST "$fa_name\n";
		print FINAL_BED "$bed_line\n";
		&getperID("Bed.fa/$file/$fa_name.fa",$prank_bin,$dir),if(-e "Bed.fa/$file/$fa_name.fa");
		if(! -e "Bed.fa/$file/$fa_name.$maf_num.fa"){
			mkdir "Join_fa/$file"; 
...
Usage:
    ...
    --tree|tr	      <string>	the tree file in nex format (note: needs absolute path)
